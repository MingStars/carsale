...
<body>
	...
	<!-- END Top Navbar--> <!-- START aside--> 
	<aside class="aside"> <!-- START Sidebar (left)--> 
	<nav class="sidebar">
	<ul class="nav">
		<!-- START user info-->
		<li>
			<div data-toggle="collapse-next" class="item user-block has-submenu">
				...
				<div class="user-block-picture">
					<img src="img/car_pic/${li.uPhoto}" alt="Avatar" width="60" height="60"
		...
		<!-- END user info-->
		<!-- START Menu-->
		<li><a href="${pageContext.request.contextPath}/UpdateUser" title="Dashboard" data-toggle=""
			class="no-submenu"> <em class="fa fa-dashboard"></em> <span
				class="UpdateUser">个人信息</span>
		</a></li>
		<li><a href="#" title="Tables" data-toggle="collapse-next"
			...
			<ul class="nav collapse ">
				<li style="background: gray;"><a href="${pageContext.request.contextPath}/BuyerUnFinsh"> <span
						class="item-text">未完成订单</span>
				</a></li>
		 ...
		 <li style="background: gray;"><a href="#" title="Tables"
			data-toggle="collapse-next" class="has-submenu"
			style="color: white; box-shadow: 0px 1px 10px rgba(0, 0, 0, .8);">
				<em class="fa fa-bar-chart-o"></em> <span class="item-text"
				style="font-size: 17px;">作为卖方</span>
		</a> <!-- START SubMenu item-->
			<ul class="nav collapse ">
				<li><a href="${pageContext.request.contextPath}/SellerSold"> <span class="item-text">已售出车辆信息</span>
				</a></li>
				<li>
				<li><a href="${pageContext.request.contextPath}/SellerDepend"> <span class="item-text">订单确认</span>
				</a></li>
				<li style="background: gray;"><a href="${pageContext.request.contextPath}/SellerNews"
					style="color: white; box-shadow: 0px 1px 10px rgba(0, 0, 0, .8);">
						<span class="item-text" style="font-size: 17px;">我发布的车辆信息</span>
				</a></li>
			</ul></li> 
		...
		<li><a href="Collection" title="Standard"> <em
				class="fa fa-table"></em> <span class="item-text">收藏夹</span>
		</a></li>
		<li><a href="${basePath}/page/index" title="Pages" data-toggle=""
	...
	<h3>
		已发布汽车信息 <br> <small>可查阅已发布的汽车信息</small>
	</h3>
	...
	<!-- dongtai --> 请输入车id： <input type="text" name="carId" id="carId">

	<!--  <input type="text" name="carName" id="carName">  --> 汽车品牌：<select
	...
	</select> 配置名：<input type="text" name="confName" id="confName"> 成色选择 <select
		id="sc" name="sc">
		<option>--请选择--</option>
		<option value="1">成色优异</option>
		<option value="2">成色很好</option>
		<option value="3">成色普通</option>


	</select> <input type="button" value="提交" id="b1"> <!-- START panel-->
	<div class="panel panel-default">
		...
		<div class="table-responsive">
				...
				<tbody id="table-tbody" class="table-tbody">
					<c:forEach items="${page.list}" var="uf">
						<tr>
							...
							<td>${uf.gearBox}</td>
							<td>${uf.engine}</td>
							<td>${uf.bodyStructure}</td>
							<td>${uf.carAge}</td>
							<td>${uf.carMileage}</td>
							<td><a href="${pageContext.request.contextPath}/DoCollection?carId=${uf.carId}">收藏</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
			...
			查询出${page.total}条 ,共${page.pages}页 <a
				href="SellerNews?pageIndex=${page.firstPage }">首页</a> <a
				href="SellerNews?pageIndex=${page.prePage }">上一页</a>

			<c:forEach items="${page.navigatepageNums }" var="i">
				<a href="SellerNews?pageIndex=${i }" <c:if test="${i==page.pageNum}">style="color: red"</c:if>>${i }</a>&nbsp;&nbsp;&nbsp;&nbsp;
	 		 </c:forEach>

			<a href="SellerNews?pageIndex=${page.nextPage }">下一页</a> <a
				href="SellerNews?pageIndex=${page.pages}">末页</a>
	...
	$(function() {
		...
		$("#b1").click(
				function() {
					...
					var sc = $("#sc").val();
					alert(carId + "***" + carName + "***" + confName + "***"
							+ sc);
					console.log("111");
					$.ajax({

						url : "../likefind?carId=" + carId + "&carName=" + carName
								+ "&confName=" + confName + "&sc=" + sc,
						...
						success : function(res) {
							...
							$.each(res.list, function(i, n) {
								trs += "<tr><td>" + n.carId + "</td>" + "<td>"
										+ n.carName + "</td>" + "<td>"
										+ n.confName + "</td>" + "<td>"
										+ n.gearBox + "</td>" + "<td>"
										+ n.engine + "</td>" + "<td>"
										+ n.bodyStructure + "</td>" + "<td>"
										+ n.carAge + "</td>" + "<td>"
										+ n.carMileage + "</td></tr>"
